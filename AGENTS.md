...
| Deployment | Netlify (build: `vite build`, publish: `dist/client`) |
...
The QR trigger page loads as vanilla HTML with zero JS framework overhead. Speed matters because users scan and walk away — any hydration delay feels broken. Netlify serves it directly from `dist/client/notify.html`.
...
netlify dev      # Netlify local emulation (port 8888, recommended)
```
